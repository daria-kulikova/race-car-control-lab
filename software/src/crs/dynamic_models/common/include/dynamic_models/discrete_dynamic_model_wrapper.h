...
class DiscreteDynamicModelWrapper : public DiscreteDynamicModel<StateType, InputType>
{
protected:
  std::unique_ptr<crs_models::ContinuousDynamicModel<StateType, InputType>> cont_model;
  casadi::Function integrator_;

public:
  DiscreteDynamicModelWrapper(Eigen::Matrix<double, StateType::NX, StateType::NX> Q)
    : DiscreteDynamicModel<StateType, InputType>(Q){};
                   ...
                   Eigen::Matrix<double, StateType::NX, InputType::NU>& D)
  {
    ...
    Eigen::Matrix<double, StateType::NX, StateType::NX> F_k2;
    Eigen::Matrix<double, StateType::NX, InputType::NU> D_k2;
    cont_model->getNumericalJacobian(state + (integration_time / 2.0) * k1, control_input, F_k2, D_k2);
    F_k2 = F_k2 * (Eigen::Matrix<double, StateType::NX, StateType::NX>::Identity() + integration_time * F_k1 / 2.0);

    Eigen::Matrix<double, StateType::NX, StateType::NX> F_k3;
    Eigen::Matrix<double, StateType::NX, InputType::NU> D_k3;
    cont_model->getNumericalJacobian(state + (integration_time / 2.0) * k2, control_input, F_k3, D_k3);
    F_k3 = F_k3 * (Eigen::Matrix<double, StateType::NX, StateType::NX>::Identity() + integration_time * F_k2 / 2.0);

    Eigen::Matrix<double, StateType::NX, StateType::NX> F_k4;
    Eigen::Matrix<double, StateType::NX, InputType::NU> D_k4;
