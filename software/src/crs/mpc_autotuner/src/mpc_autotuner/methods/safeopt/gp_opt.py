...
import numpy as np
from mpc_autotuner.methods.safeopt.swarm import SwarmOptimization
from mpc_autotuner.methods.safeopt.utilities import (
...
class GaussianProcessOptimization:
    ...
    def __init__(self, gp, fmin, beta=2, num_contexts=0, threshold=0, scaling="auto"):
        ...
        if scaling == "auto":
            ...
            if self.scaling.shape[0] != len(self.gps):
                raise ValueError(
                    "The number of scaling values should be "
    ...
    def plot(self, n_samples, axis=None, figure=None, plot_3d=False, **kwargs):
        """
        ...
        if true_input_dim == 1 or plot_3d:
            ...
            inputs[:, :true_input_dim] = linearly_spaced_combinations(
                self.bounds[:true_input_dim], n_samples
            )
        ...
        if self.gp.input_dim - self.num_contexts == 1:
            ...
            for gp, fmin in zip(self.gps, self.fmin):
                if fmin == -np.inf:
                    fmin = None
                ax = plot_2d_gp(
                    gp, inputs, figure=figure, axis=axis, fmin=fmin, **kwargs
                )
                axes.append(ax)
        else:
            if plot_3d:
                ...
                for gp in self.gps:
                    plot_contour_gp(
                        ...
                        [
                            np.linspace(
                                self.bounds[0][0], self.bounds[0][1], n_samples[0]
                            ),
                            np.linspace(
                                self.bounds[1][0], self.bounds[1][1], n_samples[1]
                            ),
                        ],
                        figure=figure,
                        axis=axis,
    ...
    def _add_data_point(self, gp, x, y, context=None):
        ...
        gp.set_XY(np.vstack([gp.X, x]), np.vstack([gp.Y, y]))

    def add_new_data_point(self, x, y, context=None) -> None:
        """
...
class SafeOpt(GaussianProcessOptimization):
    ...
    def __init__(
        ...
        if self.num_contexts > 0:
            ...
            self.inputs = np.hstack(
                (parameter_set, np.zeros(context_shape, dtype=parameter_set.dtype))
            )
        ...
        if self.liptschitz is not None:
            ...
            self.liptschitz = np.atleast_1d(np.asarray(self.liptschitz).squeeze())

        # Value intervals
        self.Q = np.empty((self.inputs.shape[0], 2 * len(self.gps)), dtype=np.float)
        self.variance = np.empty((self.inputs.shape[0]), dtype=np.float)

        # Safe set
        self.S_pessimistic = np.zeros(self.inputs.shape[0], dtype=np.bool)
        for x in self.gps[0].X:
            idx = np.linalg.norm(self.parameter_set - x, axis=1).argmin()
            self.S_pessimistic[idx] = True

        # Optimistic set, includes the parameters whose upper bound is above
        # the constraint
        # Initially the optimistic set contains all points
        # self.S_optimistic = np.ones(self.inputs.shape[0], dtype=np.bool)
        self.S_optimistic = self.S_pessimistic.copy()

        # Switch to use confidence intervals for safety
        if lipschitz is None or lipschitz > 1e3:
            self._use_lipschitz = False
        else:
            self._use_lipschitz = True

        # Set of expanders and maximizers
        self.G = self.S_pessimistic.copy()
        self.M = self.S_pessimistic.copy()
    ...
    def parameter_set(self, parameter_set):
        ...
        self.bounds = list(
            zip(
                np.min(self._parameter_set, axis=0), np.max(self._parameter_set, axis=0)
            )
        )
        self.num_samples = [
            len(np.unique(self._parameter_set[:, i]))
    ...
    def compute_safety_sets(self):
        """Compute the optimistic and the poessimistic sets based on the current confidence bounds."""
        # Update pessimistic set
        if not self.use_lipschitz or len(self.gps) > 1:
            self.S_pessimistic[:] = np.all(self.Q[:, ::2] > self.fmin, axis=1)
            self.S_optimistic[:] = np.all(self.Q[:, 1::2] > self.fmin, axis=1)
        else:
            # Get lower bounds of safe set
            l = self.Q[self.S_pessimistic, 0]
            u = self.Q[self.S_pessimistic, 1]
            d = cdist(self.inputs, self.inputs[self.S_pessimistic, :])
            new_pessimistic_set = np.any(
                l - self.liptschitz * d >= self.fmin[0], axis=1
            )
            new_optimistic_set = np.any(
                u - self.liptschitz * d - self.minimum_variance * self.beta(self.t)
                >= self.fmin[0],
                axis=1,
            )
            self.S_pessimistic = np.logical_and(
                new_pessimistic_set, self.Q[:, 0] > self.fmin[0]
            )
            self.S_optimistic = np.logical_and(
                new_optimistic_set, self.Q[:, 1] > self.fmin[0]
            )
            # self.S_pessimistic = np.logical_or(self.S_pessimistic, new_pessimistic_set)
            # self.S_pessimistic = np.logical_and(self.S_pessimistic, np.all(self.Q[:, ::2] > self.fmin, axis=1))
            # self.S_optimistic = np.logical_or(self.S_optimistic, new_optimistic_set)
            # self.S_optimistic = np.logical_and(self.S_optimistic, np.all(self.Q[:, 1::2] > self.fmin, axis=1))
            # self.S_optimistic = np.all(self.Q[:, 1::2] > self.fmin, axis=1)
        # Update optimistic set
        # self.S_optimistic[:] = np.all(self.Q[:, 1::2] > self.fmin, axis=1)
    ...
    def compute_sets(self, full_sets=False):
        ...
        # Update optimistic and pessimistic sets
        self.compute_safety_sets()

        # Reference to confidence intervals
        l, u = self.Q[:, :2].T

        if not np.any(self.S_pessimistic):
            self.M[:] = False
            self.G[:] = False
            return
        ...
        self.M[:] = False
        self.M[self.S_pessimistic] = u[self.S_pessimistic] >= np.max(
            l[self.S_pessimistic]
        )
        ...
        if full_sets:
            s = self.S_pessimistic
        else:
            # skip points in M, they will already be evaluated
            s = np.logical_and(self.S_pessimistic, ~self.M)
        ...
        for index in sort_index:
            if self.use_lipschitz:
                ...
                # points
                d = cdist(
                    self.inputs[s, :][[index], :], self.inputs[~self.S_pessimistic, :]
                )
                ...
                for i, gp in enumerate(self.gps):
                    ...
                    mean2, var2 = gp.predict_noiseless(self.inputs[~self.S_pessimistic])
    ...
    def get_new_query_point(self, ucb=True) -> Tuple[np.ndarray, bool]:
        """
        ...
        bool:
            Wheater to terminate the optimization or not.
        """
        if not np.any(self.S_pessimistic):
            raise OSError("There are no safe points to evaluate.")

        if ucb:
            # Compute the goal: UCB of the optimistic set
            goal_id = np.argmax(self.Q[self.S_optimistic, 1])
            self.x_goal = self.inputs[self.S_optimistic, :][goal_id, :]
            global_goal_id = np.argmin(
                np.linalg.norm(self.inputs - self.x_goal, axis=1)
            )
            print(f"Goal: {self.x_goal}")
            # Store its confidence interval
            goal_w = (
                self.Q[self.S_optimistic, 1][goal_id]
                - self.Q[self.S_optimistic, 0][goal_id]
            )

            # Check if the goal is inside the pessimistic set
            if self.S_pessimistic[global_goal_id]:
                # Check if the goal confidence interval is small enough
                print(f"Goal w: {goal_w}")
                print(f"Goal w min: {self.minimum_variance * self.beta(self.t)}")
                if goal_w < self.minimum_variance * self.beta(self.t):
                    # If yes, return the goal and stop
                    if self.num_contexts:
                        return self.x_goal[: -self.num_contexts], True
                    else:
                        return self.x_goal, True
                else:
                    # If not, return the goal and continue
                    if self.num_contexts:
                        return self.x_goal[: -self.num_contexts], False
                    else:
                        return self.x_goal, False
            # If the goal is not inside the pessimistic set, our sampling criterion consists of
            # steering towards the goal while ensuring that the confidence interval is small enough
            # Compute all the distances from the goal to the pessimistic set points
            distances = cdist(
                self.inputs[self.S_pessimistic, :], self.x_goal.reshape(1, -1)
            )
            # Compute the confidence intervals of the pessimistic set points
            w_pessimistic = (
                self.Q[self.S_pessimistic, 1] - self.Q[self.S_pessimistic, 0]
            )
            # Find the point with the smallest distance to the goal that has a confidence interval
            # smaller than the threshold
            candidate_points = w_pessimistic >= self.minimum_variance * self.beta(
                self.t
            )
            if np.any(candidate_points):
                candidate_id = np.argmin(distances[candidate_points])
                x = self.inputs[self.S_pessimistic, :][candidate_points, :][
                    candidate_id, :
                ]
                if self.num_contexts:
                    return x[: -self.num_contexts], False
                else:
                    return x, False
            else:
                # If no such point exists, sample at the maximum uncertainty point of the pessimistic set
                raise ValueError("No point with small enough confidence interval found")
                candidate_id = np.argmax(w_pessimistic)
                x = self.inputs[self.S_pessimistic, :][candidate_id, :]
                if self.num_contexts:
                    return x[: -self.num_contexts], False
                else:
                    return x, False
            ...
            if self.t > 10:
                condition = np.logical_and(
                    self.S_pessimistic, self.variance > self.minimum_variance
                )
                if condition.sum() == 0:
                    condition = self.S_pessimistic
            else:
                condition = self.S_pessimistic

            max_id = np.argmax(self.Q[condition, 1])
            x = self.inputs[condition, :][max_id, :]
        else:
    ...
    def optimize(self, context=None, ucb=False) -> Tuple[np.ndarray, bool]:
        ...
        if ucb:
            ...
            self.compute_sets(self.use_lipschitz)
    ...
    def get_maximum(self, context=None):
        ...
        max_id = np.argmax(l)
        return (
            self.inputs[self.S_pessimistic, :][max_id, : -self.num_contexts or None],
            l[max_id],
        )
...
class SafeOptSwarm(GaussianProcessOptimization):
    ...
    def __init__(
        self, gp, fmin, bounds, beta=2, scaling="auto", threshold=0, swarm_size=20
    ):
        ...
        self.swarms = {
            swarm_type: SwarmOptimization(
                swarm_size,
                self.optimal_velocities,
                partial(self._compute_particle_fitness, swarm_type),
                bounds=self.bounds,
            )
    ...
    def optimize_particle_velocity(self):
        ...
        velocities = np.empty((len(self.gps), self.gp.input_dim), dtype=np.float)

        for i, gp in enumerate(self.gps):
            for j in range(self.gp.input_dim):
                tmp_velocities = np.zeros((1, self.gp.input_dim), dtype=np.float)

                # lower and upper bounds on velocities
                upper_velocity = 1000.0
                lower_velocity = 0.0
                ...
                while True:
                    ...
                    if (
                        not_too_fast and velocity_enough
                    ) or upper_velocity - lower_velocity < 1e-5:
                        break
    ...
    def _compute_particle_fitness(self, swarm_type, particles):
        ...
        if is_safe:
            ...
            if is_expander:
                ...
                # the lower bounds
                interest_function = len(self.gps) * np.ones(
                    np.shape(values), dtype=np.float
                )
            elif is_maximizer:
    ...
    def get_new_query_point(self, swarm_type):
        ...
        """
        beta = self.beta(self.t)
        safe_size, input_dim = self.S_pessimistic.shape

        # Make sure the safe set is still safe
        _, safe = self._compute_particle_fitness("safe_set", self.S_pessimistic)
        ...
        if num_safe >= self.swarm_size and num_safe != len(safe):
            ...
            logging.warning(
                "Warning: {} unsafe points removed. "
                "Model might be violated".format(np.count_nonzero(~safe))
            )

            # Remove unsafe points
            self.S_pessimistic = self.S_pessimistic[safe]
            safe_size = self.S_pessimistic.shape[0]
        ...
        if swarm_type == "greedy":
            ...
            particles = self.S_pessimistic[random_id, :]
        ...
        if swarm_type != "greedy":
            ...
            covariance = self.gp.kern.K(
                swarm.best_positions,
                np.vstack((self.S_pessimistic, swarm.best_positions)),
            )
            covariance /= self.scaling[0] ** 2

            initial_safe = len(self.S_pessimistic)
            ...
            for j in range(n):
                ...
                if np.all(covariance[j, mask] <= 0.95):
                    self.S_pessimistic = np.vstack(
                        (self.S_pessimistic, swarm.best_positions[[j], :])
                    )
                    num_added += 1
                    mask[initial_safe + j] = True

            logging.debug(
                "At the end of swarm {}, {} points were appended to"
    ...
    def optimize(self, ucb=False):
        ...
        logging.info(
            "The greedy estimate of lower bound has value %f" % self.best_lower_bound
        )
