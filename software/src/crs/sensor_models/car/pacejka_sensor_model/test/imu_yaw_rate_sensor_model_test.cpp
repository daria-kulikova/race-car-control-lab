...
// ----- IMU YAW RATE TESTS -----
TEST(IMU_YAW_RATE_TEST, testForwardModel)
{
  crs_sensor_models::pacejka_sensor_models::ImuYawSensorModel sensor_model;
  ...
  for (int run = 0; run < N_RUNS; run++)
  {
    ...
    Eigen::VectorXd measurement = sensor_model.applyModel(gt_dynamic_state, current_input);

    // Check output is correct
    EXPECT_DOUBLE_EQ(measurement(0), gt_dynamic_state.yaw_rate);
  }
}

TEST(IMU_YAW_RATE_TEST, testJacobian)
{
  crs_sensor_models::pacejka_sensor_models::ImuYawSensorModel sensor_model;

  Eigen::Matrix<double, 1, 6> H_gt;
  H_gt << 0, 0, 0, 0, 0, 1;
