...
from imu_bias.get_measurement_vec import (
    get_measurement_vector as imu_bias_get_measurement_vector,
)


def generate_jacobian(
    measurement_function: Callable[
        [List[sp.Symbol], List[sp.Symbol], List[sp.Symbol]], sp.Matrix
    ],
    ...
    for i in range(10):
        ...
        state_val = np.random.rand(9) * 2 - 0.5
...
def main():
    ...
    (
        ...
        "x y yaw v_x v_y yaw_rate bias_ax bias_ay bias_yaw_rate torque steer", real=True
    )
    state = [x, y, yaw, v_x, v_y, yaw_rate, bias_ax, bias_ay, bias_yaw_rate]
    ...
    # IMU
    imu_meas_fnc = imu_bias_get_measurement_vector(state, control_input, params)
    generate_jacobian(imu_meas_fnc, state, control_input, params, "imu_bias")
