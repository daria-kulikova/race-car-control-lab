...
def generate_solver(config, lib_dir, additional_args):
    ...
    # export model
    model, constraint = safety_dynamic_model(
        config["model_bounds"], config["track_constraints"]
    )
    ...
    filename = os.path.join(lib_dir, "acados_pacjeka_safety_model_solver_config.json")
