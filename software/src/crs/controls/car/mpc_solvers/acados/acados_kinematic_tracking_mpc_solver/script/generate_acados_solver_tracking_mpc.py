...
def build_acados_solver(N: int, Ts: float, acados_source_path: str, model):
    ...
    model: Callable
        Model function e.g. kinematic_model
    ...
    filename = "lib/acados_kinematic_tracking_mpc_solver_config.json"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Create a new Acados tracking MPC solver for the kinematic model"
    )
    parser.add_argument(
        "--config",
        type=str,
        help="Name of the configuration file",
        default="solver_tracking_mpc.yaml",
    )
    parser.add_argument(
        "--acados_source",
        type=str,
    ...
    try:
        ...
        controller_path = rospack.get_path("acados_kinematic_tracking_mpc_solver")
        os.chdir(controller_path)
    except:
        print(
            "Did not find rospackage acados_kinematic_tracking_mpc_solver. Assuming script is called from the scripts folder."
        )
        os.chdir("../src")
        controller_path = "../"
    ...
    with open(os.path.join(controller_path, "config", args.config)) as f:
        ...
        def loadModel():
            return kinematic_model_tracking_mpc(cfg["model_bounds"])
