...
std::optional<std::vector<std::vector<double>>> PacejkaTrackingMpcController::getPlannedTrajectory() const
{
  ...
  for (int i = 0; i < solver_->getHorizonLength(); i++)
  {
    ...
    double x_vel = last_solution.states_[i * solver_->getStateDimension() + pacejka_tracking_vars::VX];
    double y_vel = last_solution.states_[i * solver_->getStateDimension() + pacejka_tracking_vars::VY];
