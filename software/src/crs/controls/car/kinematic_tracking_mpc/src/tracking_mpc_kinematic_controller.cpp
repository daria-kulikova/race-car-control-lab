...
std::optional<std::vector<std::vector<double>>> KinematicTrackingMpcController::getPlannedTrajectory()
{
  std::vector<std::vector<double>> traj;
  for (int i = 0; i < solver_.N; i++)
  {
    traj.push_back({
        last_solution_.x[i][kinematic_vars::X], last_solution_.x[i][kinematic_vars::Y],
        last_solution_.x[i][kinematic_vars::V],
        0.0,  // no vx
        last_solution_.x[i][kinematic_vars::YAW], last_reference_on_track_[i].x, last_reference_on_track_[i].y,
        0.0,  // no planned yaw?
    });
  }
...
void KinematicTrackingMpcController::initialize(crs_models::kinematic_model::kinematic_car_state state)
{
  ...
  StateArray x_init = { state.pos_x, state.pos_y, state.yaw, state.velocity, last_input_.torque, last_input_.steer };
  ...
  for (int run = 0; run < N_WARM_START; run++)
  {
    ...
    auto solution = solver_.solve_problem(x_init, last_solution_, parameters);
    ...
    last_input_.torque = last_solution_.x[1][kinematic_vars::TORQUE];
    last_input_.steer = last_solution_.x[1][kinematic_vars::STEER];
  }
}
