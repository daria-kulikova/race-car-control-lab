...
from pacejka_curvilinear_model import pacejka_curvilinear_model
...
def generate_solver(config, lib_dir, additional_args):
    ...
    if additional_args:
        warn(
            f"specified additional args {additional_args} to curvilinear solver. Ignoring."
        )
    ...
    # export model
    model, constraint = pacejka_curvilinear_model(config["model_bounds"])
    ...
    w = 0.5 * track_width - safety_margin
    ocp.constraints.lh = np.array([-w])
    ocp.constraints.uh = np.array([w])
    ...
    filename = os.path.join(
        lib_dir, "acados_pacejka_mpcc_curvilinear_solver_config.json"
    )
