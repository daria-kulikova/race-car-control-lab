...
# ── Trajectory on track — separate SVG ───────────────────────────────────────
svg_path = Path(out_path).with_suffix("").with_name(Path(out_path).stem + "_trajectory.svg")
fig_traj, ax_traj = plt.subplots(figsize=(7, 7))
plot_xy_seg(ax_traj, rx_b, ry_b, breaks_b, label="reference", color="black", linestyle="--", linewidth=1.0, alpha=0.5)
plot_xy_seg(ax_traj, px_a, py_a, breaks_a, label=label_a, color="tomato",    alpha=0.6, linewidth=0.8)
plot_xy_seg(ax_traj, px_b, py_b, breaks_b, label=label_b, color="steelblue", alpha=0.6, linewidth=0.8)
ax_traj.set_xlabel("x [m]")
ax_traj.set_ylabel("y [m]")
ax_traj.set_title(f"Trajectory on track: {label_a} vs {label_b}")
ax_traj.legend()
ax_traj.set_aspect("equal")
fig_traj.tight_layout()
fig_traj.savefig(svg_path, format="svg")
plt.close(fig_traj)
