// #include "acados_pacejka_mpcc_curvilinear_solver/acados_pacejka_mpcc_curvilinear_solver.h"
...
AcadosPacejkaCurvilinearMpccSolver::AcadosPacejkaCurvilinearMpccSolver()
{
  // initialize acados solver
  acados_ocp_capsule_.reset(pacejka_curvilinear_model_acados_create_capsule());
  pacejka_curvilinear_model_acados_create(acados_ocp_capsule_.get());

  nlp_config_.reset(pacejka_curvilinear_model_acados_get_nlp_config(acados_ocp_capsule_.get()));
  nlp_dims_.reset(pacejka_curvilinear_model_acados_get_nlp_dims(acados_ocp_capsule_.get()));
  nlp_in_.reset(pacejka_curvilinear_model_acados_get_nlp_in(acados_ocp_capsule_.get()));
  nlp_out_.reset(pacejka_curvilinear_model_acados_get_nlp_out(acados_ocp_capsule_.get()));
  nlp_solver_.reset(pacejka_curvilinear_model_acados_get_nlp_solver(acados_ocp_capsule_.get()));
}
 ...
 * @param constraint
 */
void AcadosPacejkaCurvilinearMpccSolver::setInitialState(AcadosPacejkaCurvilinearMpccSolver::StateArray& initial_state)
{
 ...
 * @param constraint
 */
void AcadosPacejkaCurvilinearMpccSolver::setStateInitialGuess(
    int stage, AcadosPacejkaCurvilinearMpccSolver::StateArray& initial_guess)
{
 ...
 * @param constraint
 */
void AcadosPacejkaCurvilinearMpccSolver::setInputInitialGuess(
    int stage, AcadosPacejkaCurvilinearMpccSolver::InputArray& initial_guess)
{
 ...
 */
void AcadosPacejkaCurvilinearMpccSolver::updateParams(
    int stage, AcadosPacejkaCurvilinearMpccSolver::MpcStageParameters& parameters)
{
  using params = crs_controls::pacejka_mpcc::solvers::params_curvilinear;

  std::array<double, NP> mpc_parameters;
  // Tracking
  mpc_parameters[static_cast<int>(params::KAPPA)] = parameters.auxillary_param.kappa;
  // Costs
  ...
  pacejka_curvilinear_model_acados_update_params(acados_ocp_capsule_.get(), stage, mpc_parameters.data(), NP);
}

AcadosPacejkaCurvilinearMpccSolver::MpcReturnType AcadosPacejkaCurvilinearMpccSolver::solve()
{
  using MpcExitCode = crs_controls::control_commons::MpcExitCode;

  int status = pacejka_curvilinear_model_acados_solve(acados_ocp_capsule_.get());
...
double AcadosPacejkaCurvilinearMpccSolver::getSamplePeriod()
{
  return *(nlp_in_->Ts);
}

AcadosPacejkaCurvilinearMpccSolver::MpcSolution AcadosPacejkaCurvilinearMpccSolver::getSolution()
{
  AcadosPacejkaCurvilinearMpccSolver::MpcSolution solution;
  for (int n = 0; n < N; n++)
  {
