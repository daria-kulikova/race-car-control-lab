...
crs_controls::pacejka_mpcc::mpcc_pacejka_config loadMpccParameters(std::string filename)
{
  crs_controls::pacejka_mpcc::mpcc_pacejka_config mpc_params;
  YAML::Node file_content = YAML::LoadFile(filename);
 ...
 */
template <typename MpcControllerType>
void printTrajectory(std::shared_ptr<MpcControllerType>& controller)
{
  std::ofstream myfile;
...
TEST(MpccTests, testMPCCpredictions)
{
  ...
  crs_controls::pacejka_mpcc::mpcc_pacejka_config mpc_params = loadMpccParameters("params/test_mpcc_params.yaml");

  std::shared_ptr<crs_controls::StaticTrackTrajectory> track = loadTrackDescription("params/DEMO_TRACK.yaml");

  auto controller = crs_controls::pacejka_mpcc::controllerFactory(mpc_params, model, track);
