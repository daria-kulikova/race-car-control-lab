...
loadCurvilinearTrackDescriptionFromParams(const ros::NodeHandle& nh)
{
  ...
  if (!nh.getParam("yCoords", y_coords_))
    ROS_WARN_STREAM("Could not load track! Did not find y_coords for namespace: " << nh.getNamespace() << ".");
  ...
  return std::make_shared<crs_controls::CurvilinearTrackTrajectory>(x_coords, y_coords, x_rate, y_rate, width,
                                                                    curvature, tangent_angle, arc_length, density);
}
