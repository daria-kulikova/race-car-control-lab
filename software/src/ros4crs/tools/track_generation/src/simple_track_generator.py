...
class trackGenerator:
    ...
    def addLine(self, x0, y0, x1, y1, numOfPoints):
        self.chainOfSegments = np.append(
            self.chainOfSegments, line(x0, y0, x1, y1, numOfPoints)
        )

    ##Chains an arc to the list of segments
    def addArc(self, x0, y0, radius, theta_s, theta_f, numOfPoints):
        self.chainOfSegments = np.append(
            self.chainOfSegments, arc(x0, y0, radius, theta_s, theta_f, numOfPoints)
        )
    ...
    def populatePointsAndArcLength(self):
        for segment in self.chainOfSegments:
            ...
            if type(segment) is line:
                for i in range(0, numOfPoints):
                    self.xCoords.append(
                        segment.x0 + (segment.x1 - segment.x0) / numOfPoints * i
                    )
                    self.yCoords.append(
                        segment.y0 + (segment.y1 - segment.y0) / numOfPoints * i
                    )
                    ...
                    if self.arcLength == []:
                        ...
                        self.arcLength.append(
                            segment.length / numOfPoints + self.arcLength[-1]
                        )
            if type(segment) is arc:
                for i in range(0, numOfPoints):
                    self.xCoords.append(
                        ...
                        + segment.radius
                        * np.cos(
                            (segment.theta_f - segment.theta_s) / numOfPoints * i
                            + segment.theta_s
                        )
                    )
                    self.yCoords.append(
        ...
        self.arcLength = np.append(
            self.arcLength, self.arcLength + self.arcLength[-1] + self.arcLength[1]
        )
    ...
    def pointAtArcLength(self, arcLength):
        ...
        idx = np.where(temp > 0)
        plt.plot(
            self.xCoords[idx[0][0]],
            self.yCoords[idx[0][0]],
            "ro",
            label="Point at arc length " + str(arcLength),
            markersize=1,
        )
        plt.legend()
    ...
    def plotPoints(self, ax):
        ...
        ax.plot(
            self.xCoords + self.yRate * self.track_width / 2,
            self.yCoords - self.xRate * self.track_width / 2,
            "ko",
            markersize=1,
        )
        ax.plot(
    ...
    def plotGrid(self, ax, tile_width: float):
        """Plots a grid of the tiles of the track. This can be used for reference or calibrating motion capture systems."""

        min_x, max_x = np.min(self.xCoords), np.max(self.xCoords)
        min_y, max_y = np.min(self.yCoords), np.max(self.yCoords)

        # set the ticks between the min and max coordinates, spaced by the tile width
        x_ticks = np.arange(min_x, max_x, tile_width)
        y_ticks = np.arange(min_y, max_y, tile_width)
    ...
    def plotDir(self, ax):
        ax.plot(
            self.xCoords + self.xRate,
            self.yCoords + self.yRate,
            "o",
    ...
    def straight(self, init_state, dist):
        ...
        self.addLine(x0, y0, x1, y1, self.track_density)
...
def main():
    ...
    track_function = {
        ...
        "NORA_TRACK": track_functions.nora_track,
