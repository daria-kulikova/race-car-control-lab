 ...
 * without failure, the system connects to the Wi-Fi router and enables
 * power-save mode to wait for the host computer to initiate by sending a
 * reference.
 ...
 * - In power-save mode, the system waits for new control input to arrive. As
 * soon as a reference arrives, it enables power to the actuators and starts
 * operating and sampling the sensors.
 * @{
 */
...
enum SystemState {
    ...
    PowerSave = 1,   ///< System has booted without error. Waits for input.
...
enum SystemInput {
    ...
    PauseOperation = 0x10,   ///< Didn't receive input for a while, save power
};
